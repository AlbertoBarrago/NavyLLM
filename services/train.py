...
use_bf16 = torch.cuda.is_available() and torch.cuda.get_device_capability()[0] >= 8 # BF16 requires Ampere or newer GPU (compute capability >= 8.0)
use_fp16 = torch.cuda.is_available() and not use_bf16 # Use FP16 if GPU is available but doesn't support BF16
...
trainer = Trainer(
    ...
    eval_dataset=tokenized_dataset["validation"], # Specify the validation dataset split
